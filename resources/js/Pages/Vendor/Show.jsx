...
export default function Show({ vendor, filters }) {
    ...
    useEffect(() => {
        const debounce = setTimeout(() => {
            router.get(
                ...
                route('vendors.show', vendor.data.id),
                { search },
    ...
    return (
        <AuthenticatedLayout>
            <Head title="All Vendor products" />
                ...
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="mb-8">
                        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">
                            {vendor.data.store_name} Products
                        </h1>
                    </div>
                    ...
                    {vendor.data.products?.length === 0 ? (
                        ...
                        <>
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8">
                                {vendor.data.products?.map((product) => (
                                    <ProductCard key={product.id} product={product} />
                                ))}
                            </div>

                            {/* Pagination */}
                            {vendor.data.products?.meta?.links && (
                                <div className="flex justify-center">
                                    <nav className="inline-flex -space-x-px">
                                        {vendor.data.products.meta.links.map((link, index) => (
                                            <Link
                                                key={index}
                                                href={link.url || '#'}
